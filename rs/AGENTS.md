...
| 3. 验证测试 | `cargo test --workspace` | RED 阶段：确认测试失败，旧测试仍通过 |
...
```
□ M1 [panic 风险] 所有 unwrap()/expect() 是否仅在已证明不可能失败的位置使用？
...
□ R4 [日志级别] debug/info/warn/error 的使用是否恰当？
```
...
核心管线模块（推理、人脸分析、处理器）的测试密度应高于基础设施模块。具体覆盖目标由实现者按模块复杂度自行判断，以通过 MDPR 审查为准。
...
- **库代码** (`facefusion-core`)：使用 `thiserror` 自定义语义化错误类型。凡可能失败的操作必须返回 `Result`，不允许用 `unwrap()` / `panic!` 吞没错误；纯函数（getter、构造器、纯计算）不受此限
- **`expect(msg)`** 仅限已证明不可能失败的位置使用，须注释说明证明理由
...
遵循 Rust 标准命名惯例（RFC 430）。Crate 名 `kebab-case`，模块和文件名 `snake_case`，类型名 `PascalCase`，函数和变量 `snake_case`，常量和静态量 `SCREAMING_SNAKE_CASE`。Trait 名使用动词或 -er 后缀。
